...
| 「振り返り」「レトロ」「先週どうだった?」 | `10_projects/active/` と `memory/MEMORY.md` を参照してレビュー | `/weekly-review` |
| 「ベンチマーク確認して」 | `_benchmarks/_summary.md` を参照して関連示唆を提示 | 直接実行 |
| 「オペレーション」「店舗」「在庫」「シフト」「EC運営」 | オペレーション全般の確認・対応 | `/ops-agent` |
| 「マーケティング」「SNS」「コンテンツ」「キャンペーン」「投稿」 | マーケ施策の立案・実行 | `/marketing-agent` |
| 「売上」「財務」「P&L」「粗利」「キャッシュ」「予算」 | 財務数値の確認・分析 | `/finance-agent` |
| 「商品」「仕入れ」「蔵元」「在庫発注」「ソーシング」 | 商品・調達の管理 | `/product-agent` |
| 「B2B」「法人」「提案書」「クライアント」「卸」 | 法人営業の管理・推進 | `/b2b-agent` |
